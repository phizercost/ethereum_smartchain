...
describe("Block", () => {
  ...
  describe("validateBlock()", () => {
    ...
    it("rejects when the parent hash is invalid", () => {
      ...
      expect(Block.validateBlock({ lastBlock, block, state })).rejects.toMatchObject({
        message: "The parent hash must be hash of the last block's headers"
      });
    });
    ...
    it("rejects when the proof of work requirement is not met", () => {
      const originalCalculatedBlockeTargetHash = Block.calculateBlockTargetHash;
      Block.calculateBlockTargetHash = () => {
        return "0".repeat(64);
      };
      expect(Block.validateBlock({ lastBlock, block,state })).rejects.toMatchObject({
