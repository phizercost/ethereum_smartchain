  ...
  static validateStandardTransaction({ state, transaction }) {
    return new Promise((resolve, reject) => {
      ...
      if((value + gasLimit) > fromBalance){
        return reject(new Error(`Transaction value and gas limit:${value} exceeds balance:S{fromBalance}`));
      }
      ...
      if(toAccount.codeHash){
        ...
        if (gasUsed > gasLimit){
          return reject(new Error(`Transaction needs more gas. Provided: ${gasLimit}. Needs:${gasUsed}`));
        }
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      fields.forEach(field => {
        if (!expectedAccountDataFields.includes(field)) {
          return reject(
            new Error(`The field: ${field} is unexpected for accound data`)
          );
        }
      });
        ...
        try {
          switch (transaction.data.type) {
            ...
            case TRANSACTION_TYPE_MAP.TRANSACT:
              await Transaction.validateStandardTransaction({ state, transaction });
              break;

            case TRANSACTION_TYPE_MAP.MINING_REWARD:
                await Transaction.validateMiningRewardTransaction({ state, transaction });
                break;
            default:
              break;
          }
